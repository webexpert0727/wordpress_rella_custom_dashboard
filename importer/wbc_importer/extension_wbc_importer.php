...
if ( !class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
    ...
    class ReduxFramework_extension_wbc_importer {
        ...
        public function ajax_importer() {
            ...
            if ( isset( $_REQUEST['demo_import_id'] ) ) {

               
				
				if( !is_array($_REQUEST['selections']) || !in_array("content", $_REQUEST['selections']) )  $this->import_content = false;
		
				if( !is_array($_REQUEST['selections']) || !in_array("media", $_REQUEST['selections']) )  $this->import_media = false;
				
				if( !is_array($_REQUEST['selections']) || !in_array("widgets", $_REQUEST['selections']) )  $this->import_widgets = false;
				
				if( !is_array($_REQUEST['selections']) || !in_array("settings", $_REQUEST['selections']) )  $this->import_settings = false;
