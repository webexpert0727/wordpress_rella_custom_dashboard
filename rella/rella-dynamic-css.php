...
* The Rella_Dynamic_CSS initiate dynamic css.
*/

if( !defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

class Rella_Dynamic_CSS extends Rella_Base {
	...
	public function __construct() {
		...
		$opt_name = rella()->get_option_name();
	...
	public function set_mode() {
		...
		$mode = rella_helper()->get_theme_option( 'dynamic_css_compiler' ) ? 'file' : 'inline';
		...
		if ( 'file' == $mode && $this->needs_update() ) {
			...
			$last_time    = (int) get_option( 'rella_dynamic_css_time' );
	...
	public function make_css() {
		...
		if ( ! $wp_filesystem->put_contents( $this->file( 'path' ), $content, FS_CHMOD_FILE ) ) {
			...
			$option  = get_option( 'rella_dynamic_css_posts', array() );
			$option[$page_id] = true;
			update_option( 'rella_dynamic_css_posts', $option );
	...
	public function needs_update() {
		...
		// Get the 'rella_dynamic_css_posts' option from the DB
		$option  = get_option( 'rella_dynamic_css_posts', array() );
		...
		// If the current page ID exists in the array of pages defined in the 'rella_dynamic_css_posts' option
	...
	public function can_write() {
		...
		$file_name   = '/rella-css' . $blog_id . '-' . $page_id . '.css';
		$folder_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'rella-styles';
	...
	public function file( $target = 'path' ) {
		...
		$css_uri = trailingslashit( $css_uri_folder ) . 'rella-styles/' . $file_name;
	...
	public function dynamic_css_cached() {
		 ...
		 */
		$cache = ( $cache && ( null == rella_helper()->get_theme_option( 'dynamic_css_db_caching' ) || ! rella_helper()->get_theme_option( 'dynamic_css_db_caching' ) ) ) ? false : $cache;
		...
		if( $cache ) {
			...
			/**
		 	 ...
		 	 */
			$transient_name = $page_id ? 'rella_dynamic_css_' . $page_id : 'rella_dynamic_css_global';
	...
	public function dynamic_css() {
		 ...
		 */
		$dynamic_css = $this->dynamic_css_parser( rella_dynamic_css_array() );

		/**
		 * Append the user-entered dynamic CSS
		 */
		$dynamic_css .= wp_strip_all_tags( rella_helper()->get_theme_option( 'custom_css' ) );
	...
	public function dynamic_css_parser( $css ) {
		...
		return apply_filters( 'rella_dynamic_css', $final_css );
	}
	...
	public function enqueue_dynamic_css() {
		...
		if ( 'file' == self::$mode ) {
			...
			wp_enqueue_style( 'rella-dynamic-css', $this->file( 'uri' ) );
		}
	}
	...
	public function add_inline_css() {
		...
		if ( ('inline' == self::$mode || $wp_customize) && $dynamic_css = $this->dynamic_css_cached() ) {
			echo "<style id='rella-stylesheet-inline-css' type='text/css'>" . $dynamic_css . '</style>';
		}
	...
	public function reset_post_transient( $post_id ) {
		delete_transient( 'rella_dynamic_css_' . $post_id );
	}

	/**
	 * Update the rella_dynamic_css_posts option when a post is saved.
	 */
	...
	public function add_options() {
		/**
		 * The 'rella_dynamic_css_posts' option will hold an array of posts that have had their css generated.
		 * We can use that to keep track of which pages need their CSS to be recreated and which don't.
		 */
		add_option( 'rella_dynamic_css_time', array(), '', 'yes' );
		/**
		 * The 'rella_dynamic_css_time' option holds the time the file writer was last used.
		 */
		add_option( 'rella_dynamic_css_time', time(), '', 'yes' );
	}
	...
	public function clear_cache() {
		...
		if ( rella_helper()->get_theme_option( 'dynamic_css_compiler' ) && rella_helper()->get_theme_option( 'cache_server_ip' ) ) {
	...
	public function clear_varnish_cache( $url ) {
		...
		if ( rella_helper()->get_theme_option( 'cache_server_ip' ) ) {
			$varniship = rella_helper()->get_theme_option( 'cache_server_ip' );
	...
	/**
	 * Update the rella_dynamic_css_posts option when the theme options are saved.
	 * This basically empties the array of page IDS from the 'rella_dynamic_css_posts' option
	 */
	public function global_reset_option() {
		update_option( 'rella_dynamic_css_posts', array() );
	}
	...
	public function update_saved_time() {
		update_option( 'rella_dynamic_css_time', time() );
	}
}
